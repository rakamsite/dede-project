...
$billing_title = 'company' === $role ? 'آدرس دفتر مرکزی' : ('store' === $role ? 'آدرس محل فعالیت' : 'آدرس محل سکونت');
$same_address_label = 'company' === $role ? 'همان آدرس دفتر مرکزی' : ('store' === $role ? 'همان آدرس محل فعالیت' : 'همان آدرس محل سکونت');
$identifier_label = 'company' === $role ? 'شناسه ملی' : 'کد ملی';
$city_map_json = wp_json_encode(
    $cities_by_state,
    JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE
);
?>
<div id="<?php echo 'account' === $context ? 'Information' : 'dede-checkout-profile'; ?>"
     class="dede-profile dede-profile--<?php echo esc_attr($context); ?> <?php echo 'account' === $context ? 'hidden' : ''; ?>"
     data-dede-profile
     data-start-step="<?php echo esc_attr($start_step); ?>"
     data-context="<?php echo esc_attr($context); ?>"
     data-account-role="<?php echo esc_attr($role); ?>"
     data-same-address-label="<?php echo esc_attr($same_address_label); ?>"
     data-identifier-label="<?php echo esc_attr($identifier_label); ?>"
     data-complete="<?php echo $is_complete ? '1' : '0'; ?>">
    <header class="dede-profile__header">
        <div class="dede-profile__heading">
            <h2><?php echo 'checkout' === $context ? 'تکمیل اطلاعات خرید' : 'اطلاعات حساب کاربری'; ?></h2>
        </div>
        <div class="dede-profile__meta">
            <button type="button" class="dede-profile__account-type" data-open-account-type>
                <span>نوع حساب: <?php echo esc_html($role_labels[$role]); ?></span>
                <b>تغییر</b>
            </button>
    ...
    <form class="dede-profile__form" novalidate>
        ...
        <div class="dede-profile__panel" data-step="1">
            <div class="dede-profile__section-heading"><h3><?php echo esc_html($identity_title); ?></h3></div>
                ...
                <?php else : ?>
                    ...
                    <label class="dede-field">
                        ...
                        <select name="gender" required>
                            ...
                            <option value="آقای" <?php selected($profile['gender'], 'آقای'); ?>>آقا</option>
                            <option value="خانم" <?php selected($profile['gender'], 'خانم'); ?>>خانم</option>
                        </select>
        ...
        <div class="dede-profile__panel" data-step="2">
            <div class="dede-profile__section-heading"><h3>اطلاعات آدرس</h3></div>
            ...
            <section class="dede-address-block">
                ...
                <div class="dede-profile__grid">
                    ...
                    <label class="dede-field dede-field--wide">
                        ...
                        <textarea name="billing_address_1" rows="2" autocomplete="street-address" required><?php echo esc_textarea($profile['billing_address_1']); ?></textarea>
            ...
            <label class="dede-same-address">
                ...
                <span><strong>آدرس ارسال با <?php echo esc_html($billing_title); ?> یکسان است</strong><small>اطلاعات آدرس ارسال خودکار ذخیره می‌شود.</small></span>
            </label>
            ...
            <section class="dede-address-block dede-address-block--shipping" data-shipping-fields>
                ...
                <div class="dede-profile__grid">
                    ...
                    <label class="dede-field dede-field--wide">
                        <span>آدرس کامل <b>ضروری</b></span>
                        <textarea name="shipping_address_1" rows="2" required><?php echo esc_textarea($profile['shipping_address_1']); ?></textarea>
        ...
        <div class="dede-profile__panel" data-step="3">
            <div class="dede-profile__section-heading"><h3>بررسی و تأیید اطلاعات</h3></div>

            <div class="dede-review">
                <article><div><span>نوع حساب</span><strong><?php echo esc_html($role_labels[$role]); ?></strong></div><button type="button" data-open-account-type>ویرایش</button></article>
    ...
    <script type="application/json" data-dede-city-map><?php echo $city_map_json; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></script>

    <div class="dede-account-type-modal dede-account-type-modal--change" data-account-type-modal hidden>
        <div class="dede-account-type-modal__dialog" role="dialog" aria-modal="true" aria-label="تغییر نوع حساب کاربری">
            <button type="button" class="dede-account-type-modal__close" data-close-account-type aria-label="بستن">×</button>
            <?php dede_store_features()->account_type()->render('change', $role); ?>
        </div>
    </div>
</div>
