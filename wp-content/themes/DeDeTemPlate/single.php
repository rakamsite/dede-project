...
while (have_posts()) :
    ...
    $custom_template = get_post_meta($post_id , '_custom_template' , true);
        ...
        <main id="main">
            ...
            <section class="container mx-auto grid md:grid-cols-3 mt-5 gap-5 relative">
                <section class="md:col-span-2">
                    <article>
                        ...
                        <div class="p-5">
                            <h1 class="entry-title text-xl"><?php the_title(); ?></h1>
                        </div>
                        <div class="w-full text-justify prose prose-a:text-[#0058BF] prose-a:no-underline prose-ul:list-inside prose-ol:list-inside !max-w-full break-words px-5 md:px-0">
                            <div class="flex flex-wrap md:flex-nowrap w-full">
                                <img class="md:w-96 md:h-96 w-full aspect-square rounded-lg block ml-4 mb-5"
                                     src="<?php echo $thumbnail ?>" alt="<?php the_title() ?>"/>
                                <?php the_excerpt(); ?>
                            </div>
                            <div id="content-menu"
                                 class="w-full max-h-60 rounded-2xl p-4 pt-0 bg-[#F2F2F2] mb-5 overflow-y-auto overflow-x-hidden">
                                <p class="font-bold text-lg sticky top-0 bg-[#F2F2F2]">فهرست محتوا</p>
                            </div>
                            <?php if (wp_is_mobile()) {
                                echo related_product_generator($video_related_product);
                            } ?>
                    ...
                    <div class="px-4 md:p-0">
                        <?php if (!empty($button_text) && !empty($button_link) && !empty($button_img)) {
                            echo <<<HTML
                            <a href="$button_link" target="_blank" class="flex justify-between px-3 py-2 items-center bg-[#2F2483] md:rounded-lg md:my-5 fixed md:static bottom-0 right-0 w-full z-[999]">
                                <div class="flex items-center text-white gap-2">
                                    <button class="block md:hidden" id="removeButtonInMobile">
                    ...
                    <section class="py-10 px-4">
                        ...
                        if (comments_open() || get_comments_number()) {
                            comments_template('template/video-comment.php');
                        }
                        ?>
                    </section>
                    <?php if (wp_is_mobile()): ?>
                ...
                <?php if (!wp_is_mobile()): ?>
                    <aside>
                        <div class="border p-5 rounded-lg border-gray-700 sticky top-0 bg-white overflow-y-visible">
            ...
            <section class="w-full">
                <div class="w-full bg-[#F2F2F2] mb-5 under_header">
                    <div class="container mx-auto bg-[#F2F2F2] rounded-lg">
                        <div class="py-3 w-fit px-5">
                            <div class="flex gap-2 text-sm">
                                <a href="<?php echo home_url('/') ?>" class="text-[#0058BF]">صفحه
                                    اصلی</a>
